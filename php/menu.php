...
//construction tableau RAYON
	$resultats = $connexion->prepare("SELECT * FROM rayon");
...
//construction tableau CATEGORIE
	$resultats = $connexion->prepare("SELECT * FROM categorie");
	$resultats->setFetchMode(PDO::FETCH_OBJ);
...
//AFFICHAGE des listes
	...
	{
		echo'<li><a href="#" onclick="requestRayon('.$rayon[0][$i].', \''.addslashes(htmlentities($rayon[1][$i])).'\');">'.
					'<img src="'.$rayon[2][$i].'">'.
					'<p>'.$rayon[1][$i].'</p></a>'.
				'<ul>';
